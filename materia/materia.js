...
(function () {
  ...
  /* Сторона плитки зерна в пикселях. Число не декоративное: по нему
     округляется сдвиг слоя, и ошибиться в нём — значит возить зерно
     трансформом, который каждый раз меняет картинку. */
  var ЗЕРНО = 128;
  ...
  function vesti(canvas) {
    ...
    /* Зерно печатается на бумагу умножением — но не режимом смешивания.
       Раньше здесь стояло mix-blend-mode:multiply, и это была самая дорогая
       строка во всём файле: слой во весь экран, который смешивается с
       подложкой, заставляет браузер собирать её в отдельную поверхность и
       пересчитывать смешение при каждой прокрутке. Замерено на оседании
       картинки после прыжка на экран (1280×900, ×4): 285 мс с ним, 195 без.

       Умножение при этом никуда не делось, просто оно теперь в самой
       картинке. Умножить фон B на серое g — то же самое, что положить сверху
       чёрное с прозрачностью a = 1 − g: B·(1−a) = B·g. Тождество, не
       приближение; поэтому grana.png хранит чёрный с альфой, а не серый.
       Кто будет перерисовывать плитку — считайте альфу так же.

       В покое замена точна до байта: снимки экрана до и после совпали
       целиком. Под прогибом расхождение есть — зерно выходит примерно на
       процент темнее (в среднем 4 уровня из 255, самое большое 7), потому
       что feDisplacementMap пересчитывает уже помноженную на альфу
       картинку. Проверено отдельно: при спрятанном зерне снимки под
       прогибом совпадают. Процент на текстуре, вся амплитуда которой
       одиннадцать процентов, не виден; сказано об этом здесь потому, что
       «тождество» без этой оговорки было бы неправдой. */
    /* Зерно ездит трансформом, округлённым до плитки, и лежит на своём слое —
       тот же приём, что у клетки, и по той же причине: плитка периодична с
       шагом 128 пикселей, поэтому сдвиг, кратный ста двадцати восьми, менять
       нечего, а остаток отдаётся композитору. Раньше зерно возилось
       background-position, и каждый кадр прокрутки перекрашивал слой во весь
       экран: 191 мс растеризации на десять прыжков против 148.

       Слой поэтому на плитку ниже окна: без запаса сдвиг вверх обнажал бы
       полосу у нижнего края. */
    grana.style.cssText =
      'position:fixed;inset:0 0 -' + ЗЕРНО + 'px 0;z-index:-1;pointer-events:none;' +
      'background-repeat:repeat;background-size:' + ЗЕРНО + 'px ' + ЗЕРНО + 'px;' +
    ...
    regole.textContent =
      /* Перспективы здесь нет, и это не забывчивость. Она нужна наклону
         листа, наклон живёт только под нажатым курсором, а стоит перспектива
         не за время наклона, а всегда: она заводит для всего содержимого
         трёхмерный контекст, и прокрутка перестаёт быть дешёвой. На
         тачскрине это уже было замерено на свайпе (Pixel 7, ×4): 82 кадра
         длиннее 32 мс против 64 без неё, — и там её выключили совсем. На
         мыши она оставалась постоянно и стоила ровно того же: оседание
         картинки после прыжка на экран (1280×900, ×4) — 350 мс с ней, 285
         без. Поэтому теперь её включает piega() на время прогиба и снимает,
         когда лист снова плоский. Разрыва не видно: в тот момент, когда она
         появляется, наклон ещё нулевой. */
  ...
  function start(canvas, parti) {
    ...
    function piega(finito) {
      ...
      /* Перспектива и её точка схода — тоже мышиная роскошь, и обе ставятся
         прямо в стиль .scena, а не через переменную. Переменная наследуется,
         поэтому запись в неё обесценивает стили всего, что внутри: на витке
         это две тысячи элементов и 27 мс пересчёта. Каждый кадр.
         perspective-origin не наследуется — пересчитывается только .scena. */
      if (!coarse) {
        стиль(scena, 'perspective', finito ? '' : '900px');
        стиль(
          scena,
          'perspectiveOrigin',
          finito ? '' : (pressoX * 100).toFixed(1) + '% ' + (pressoY * 100).toFixed(1) + '%',
        );
      }
      ...
      /* Зерно стоит на своём слое, сдвинутом вверх на остаток от плитки, —
         значит его собственные координаты уехали от экранных ровно на этот
         остаток, и область фильтра с местом пальца надо поправить, иначе
         прогиб зерна разойдётся с прогибом листа на сотню пикселей. */
      var сдвигЗерна = window.scrollY % ЗЕРНО;
      regione('piega-foglio', 0, window.scrollY - 40, w, h + 80);
      regione('piega-grana', 0, сдвигЗерна, w, h);
      ...
      posa(mappaGrana, cx, cy + сдвигЗерна);
    ...
    function scorri() {
      ...
      стиль(grana, 'transform', 'translateY(' + -(y % ЗЕРНО) + 'px)');
