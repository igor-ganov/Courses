...
export interface BlockDefinition<T extends { kind: string } = { kind: string }> {
   ...
   *
   * Нужно ровно для одного: задание вида «цель в приборе» называет цель по
   * имени, и если рядом нет прибора, который это имя произносит, задание
   * невыполнимо — а выглядит совершенно обычно. Проверка стоит в тестах
   * содержания, и без этого поля ей неоткуда было бы взяться, кроме как из
   * зашитого списка внутри проверки.
   *
   * Спрашивается о блоке, а не о виде, потому что у одних приборов цель
   * настраивается (контур молчит, пока в нём не задан допуск), а у других она
   * своя по построению (чёрный ящик всегда про опознание).
   */
  readonly goals?: (block: T) => readonly string[];
}
