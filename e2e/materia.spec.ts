...
test('во время прокрутки лист не под фильтром', async ({ page }) => {
  await разбудить(page);
  ...
  /* Палец лежит на листе и ведёт страницу. Пока непонятно, нажатие это или
     листание, лист успевает тронуться — пружина за два кадра проходит
     ничтожную долю хода, и глазу этого не видно. А дальше страница поехала,
     и прогиб перестаёт считаться: по летящему тексту его всё равно не
     прочесть, а фильтр во весь экран стоит кадра. */
  const замеры: string[] = [];
  for (let i = 1; i <= 12; i += 1) {
    await cdp.send('Input.dispatchTouchEvent', {
      type: 'touchMove',
      touchPoints: [{ x: 200, y: 700 - i * 30 }],
    });
    замеры.push(await page.evaluate(фильтр));
  }
  await cdp.send('Input.dispatchTouchEvent', { type: 'touchEnd', touchPoints: [] });

  const поехали = замеры.slice(3);
  expect(поехали.join(' '), замеры.join(' ')).not.toContain('piega-foglio');
});
