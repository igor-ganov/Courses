...
(function () {
  ...
  /* Форма поля не зависит от того, на какой высоте палец: край, который её
     ...
     расстояние до боковых полей (с шагом в сорок пикселей) и куда развёрнут
     сектор; вертикальный свайп идёт на одной карте. Больше десятка не держим. */
  var кэш = {};
  var кэшРазмер = 0;

  function mappaCono(cx, w, giu) {
    var ключ = Math.round(cx / 40) + ':' + Math.round(w / 40) + ':' + (giu > 0 ? 'g' : 's');
    ...
    // Карта считается на огрублённое место — на нём же её потом и ставят,
    // иначе вершина конуса разъедется с пальцем.
    cx = Math.max(0, Math.min(w, Math.round(cx / 40) * 40));
    var asse = giu;
